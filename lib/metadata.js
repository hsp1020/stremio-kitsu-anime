...
async function toStremioCatalogMeta(animeData) {
  ...
  return {
    ...
    genres: meta.genres,
    logo: '',  // fanart 완전 제거
    poster: meta.poster,  // kitsu 데이터만 사용
    background: meta.background,  // kitsu 데이터만 사용
...
async function _toStremioMeta(animeData) {
  ...
  return cleanOutputObject({
    ...
    genres: genres || [],
    logo: '', // fanart 완전 제거
    poster: animeData?.posterImage?.medium || null, // kitsu 데이터만 사용
    background: animeData?.coverImage?.original || null, // kitsu 데이터만 사용
...
function kitsuLinks(animeData, type) {
  ...
  const franchise = (animeData.mediaRelationships.data || [])
    .filter(r => allowedRelationships.includes(r.role))
    .map(r => ({
      name: `${capitalize(r.role)}: ${r.destination.data.canonicalTitle}`,
      category: 'Franchise',
      url: `stremio:///detail/${type}/kitsu:${r.destination.data.id}`
    }));
